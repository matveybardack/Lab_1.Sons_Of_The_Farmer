...
namespace WinFormsAppShapes
{
    public partial class FormFermerTask : Form
    {
        ...
        private string[] newItemList(int sides)
        {
            ...
            bool iskl = false;
            //Цикл в котором составляется новый массив значений, которые будут в списке. При попытке добавить то же число - число не добавляется
            for (int i = 0; i < finalLength; i++)
            {
                if (comboBoxAngles.Items[i].ToString() == "Другое значение")
                {
                    k = i+1;
                    items[i] = sides.ToString();
                    break;
                }
                if (sides == Convert.ToInt32(comboBoxAngles.Items[i]))
                {
                    iskl = true;
                    finalLength -= 1;
                    k = i; 
                ...
                if (Convert.ToInt32(comboBoxAngles.Items[i]) <= sides)
                {
            ...
            if (iskl)
            {
                for (int i = k; i < finalLength + 1; i++)
                    items[i - 1] = comboBoxAngles.Items[i - 1].ToString();
            }
            else
            {
            ...
            Array.Resize(ref items, finalLength);
            return items;
        }
        /// <summary>
